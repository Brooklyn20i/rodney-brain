...
describe('rest timer persistence', () => {
  ...
  it('round-trips an absolute deadline for the same workout', () => {
    ...
    const snap = loadRestTimer(storage, 'w1');
  ...
  it('ignores a snapshot from a different workout', () => {
    const now = 1_000_000;
    saveRestTimer(storage, { workoutId: 'w1', endsAt: now + 180_000, total: 180 });
    expect(loadRestTimer(storage, 'w2')).toBeNull();
  });

  it('restores a completed timer no matter how old, so save/reset state is visible after mobile resume', () => {
    const now = 1_000_000;
  ...
  it('clears on demand and tolerates corrupt data', () => {
    ...
    expect(loadRestTimer(storage, 'w1')).toBeNull();
