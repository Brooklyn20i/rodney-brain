...
export function Nutrition({ onMenu }: { onMenu: () => void }) {
  ...
  // Quick add form. Meal slots default to the time of day — logging "now"
  // should need zero corrections for the common case.
  ...
  // The picker is the PRIMARY logging path (repeat meals dominate), so it is
  // open by default and sits above the manual form.
  const [showSaved, setShowSaved] = useState(true);
  // Which meal a saved-food log lands in. The food's stored meal is just where
  // it was first logged; time-of-day is the better default and stays editable.
  const [logMeal, setLogMeal] = useState<MealType>(() => mealForHour(new Date().getHours()));
  // Double-tap guards + brief "logged ✓" confirmations. Without the guard a
      ...
      <div className="screen-content">
        ...
        <Card title="Quick add">
          ...
          <button
            ...
            onClick={quickAdd}
            disabled={addBusy || !name.trim() || !Number(cals)}
          >
