...
function generateHtml(data: MeetingData, title: string, person: Person, createdAt: string, tab: Tab): string {
  ...
  const agendaSection = (() => {
    ...
    const itemHtml = (icon: string, a: { title: string; notes: string }) =>
      `<div style="display:flex;gap:10px;margin-bottom:9px;align-items:flex-start;">
        ...
        <div>
          <div style="font-family:Calibri,Segoe UI,sans-serif;font-size:14px;font-weight:600;">${escapeHtml(a.title)}</div>
          ${a.notes ? `<div style="font-family:Calibri,Segoe UI,sans-serif;font-size:12px;color:#666;margin-top:2px;font-style:italic;">${sanitizeHtml(a.notes)}</div>` : ''}
        </div>
      </div>`;
    const deferredBlock = deferred.length
      ? `<div style="background:#FFF8EE;border-radius:6px;padding:9px 12px;margin-top:8px;border-left:3px solid #E07D00;">
          ...
          ${deferred.map((a) => `<div style="font-family:Calibri,Segoe UI,sans-serif;font-size:13px;color:#E07D00;">⏭ ${escapeHtml(a.title)}</div>`).join('')}
        </div>` : '';
  ...
  const actionsSection = (() => {
    ...
    const rows = data.actions.map((a) => {
      ...
      return `<div style="display:flex;gap:10px;padding:9px 12px;border-radius:7px;background:#F8F8F6;border-left:3px solid ${borderColor};margin-bottom:8px;align-items:flex-start;">
        ...
        <div>
          <div style="font-family:Calibri,Segoe UI,sans-serif;font-size:14px;font-weight:600;${a.done ? 'text-decoration:line-through;opacity:0.6;' : ''}">${escapeHtml(a.title)}</div>
  ...
  const notesSection = (() => {
    ...
    return sectionHead('Notes') + `<div style="font-family:Calibri,Segoe UI,sans-serif;font-size:13px;line-height:1.7;color:#333;">${sanitizeHtml(body)}</div>`;
  })();
...
<body style="font-family:Calibri,'Segoe UI',sans-serif;max-width:720px;margin:0 auto;padding:28px 24px;color:#1A1A1A;background:#fff;">
  ...
  <div style="font-size:13px;color:#666;padding-bottom:14px;border-bottom:2px solid #217346;margin-bottom:2px;">${dateStr} &nbsp;·&nbsp; ${escapeHtml(person.name)}</div>
...
function generateOneNoteHtml(data: MeetingData, person: Person, createdAt: string): string {
  ...
  if (covered.length || discuss.length || deferred.length) {
    ...
    covered.forEach((a) => {
      const sub = a.notes ? `<ul style="padding-left:14px;margin:2px 0;"><li style="color:#555;font-style:italic;">${sanitizeHtml(a.notes)}</li></ul>` : '';
      parts.push(`<li>✅ <b>${escapeHtml(a.title)}</b>${sub}</li>`);
    });
    discuss.forEach((a) => {
      const sub = a.notes ? `<ul style="padding-left:14px;margin:2px 0;"><li style="color:#555;font-style:italic;">${sanitizeHtml(a.notes)}</li></ul>` : '';
      parts.push(`<li>💬 <b>${escapeHtml(a.title)}</b>${sub}</li>`);
    ...
    deferred.forEach((a) => {
      const sub = a.notes ? `<ul style="padding-left:14px;margin:2px 0;"><li style="color:#555;font-style:italic;">${sanitizeHtml(a.notes)}</li></ul>` : '';
      parts.push(`<li>⏭ <b>${escapeHtml(a.title)}</b> — deferred${sub}</li>`);
    });
    parts.push('</ul>');
  }
  ...
  if (meetingNotes && meetingNotes !== '<p></p>') {
    ...
    parts.push(`<div style="${f}font-size:13px;line-height:1.6;color:#333;">${sanitizeHtml(meetingNotes)}</div>`);
  }
