-- Cadence Financial — agent_messages
--
-- App-side half of the Kobe integration: a message channel Rodney's agents
-- (Kobe/Warren/Dan, running in his separate Hermes environment) can read
-- and post to. This migration only creates the table + RLS; it does not
-- grant any agent access. To actually connect Kobe, mirror Cadence's own
-- pattern (Cadence/backend/AGENT_ACCESS_RUNBOOK.md): create a dedicated
-- non-owner Supabase auth account for the agent, then grant it row access
-- via a policy analogous to Cadence's `cadence_agent_access` table, scoped
-- to this owner_id only. That grant is a follow-up, done outside this repo.
